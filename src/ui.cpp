...
void InitListViewColumns(HWND list){
    const wchar_t* cols[]={L"序号",L"文件名",L"大小",L"下载地址",L"状态"}; int w[]={60,260,120,620,120};
    for(int i=0;i<5;++i){ LVCOLUMNW c{}; c.mask=LVCF_TEXT|LVCF_WIDTH|LVCF_SUBITEM; c.pszText=(LPWSTR)cols[i]; c.cx=w[i]; c.iSubItem=i; ListView_InsertColumn(list,i,&c);} }
void CreateMainUi(HWND hwnd,HINSTANCE h,UiHandles& ui){
    ...
    ui.loadBtn=CreateWindowW(L"BUTTON",L"扫描文件",WS_CHILD|WS_VISIBLE,0,0,0,0,hwnd,(HMENU)102,h,nullptr);
    ui.loadLocalBtn=CreateWindowW(L"BUTTON",L"全部下载",WS_CHILD|WS_VISIBLE,0,0,0,0,hwnd,(HMENU)108,h,nullptr);
    ...
    ui.chooseDirBtn=CreateWindowW(L"BUTTON",L"选择保存目录",WS_CHILD|WS_VISIBLE,0,0,0,0,hwnd,(HMENU)112,h,nullptr);
    ...
    ui.downloadBtn=CreateWindowW(L"BUTTON",L"下载选中",WS_CHILD|WS_VISIBLE,0,0,0,0,hwnd,(HMENU)104,h,nullptr);
    ui.cancelBtn=CreateWindowW(L"BUTTON",L"取消下载",WS_CHILD|WS_VISIBLE,0,0,0,0,hwnd,(HMENU)105,h,nullptr);
    ui.openDirBtn=CreateWindowW(L"BUTTON",L"打开保存目录",WS_CHILD|WS_VISIBLE,0,0,0,0,hwnd,(HMENU)109,h,nullptr);
    ...
    ui.listMenu=CreatePopupMenu(); AppendMenuW(ui.listMenu,MF_STRING,1201,L"下载选中"); AppendMenuW(ui.listMenu,MF_STRING,1202,L"复制下载地址"); AppendMenuW(ui.listMenu,MF_STRING,1203,L"打开保存目录"); AppendMenuW(ui.listMenu,MF_STRING,1204,L"清空状态");
...
void ResizeMainUi(HWND hwnd,UiHandles& u){ RECT r{}; GetClientRect(hwnd,&r); int w=r.right,h=r.bottom;
    MoveWindow(u.urlEdit,M,M,w-640,H,TRUE); MoveWindow(u.loadBtn,w-620,M,100,H,TRUE); MoveWindow(u.loadLocalBtn,w-510,M,100,H,TRUE);
