...
private: static LRESULT CALLBACK WndProc(HWND,UINT,WPARAM,LPARAM); LRESULT Handle(UINT,WPARAM,LPARAM);
 void LoadFromUrl(); void LoadFromLocal(); void Populate(); void SetStatus(int row,const std::wstring& s); void StartDownload(); void Cancel(); void OpenDownloads();
 HINSTANCE h_; HWND hwnd_{}; UiHandles ui_{}; std::vector<VersionItem> items_; DownloadWorker worker_; std::wstring downloadDir_=L"downloads";};
