...
bool Utf8ToWide(const std::string& in, std::wstring& out){int wlen=MultiByteToWideChar(CP_UTF8,0,in.data(),(int)in.size(),nullptr,0); if(wlen<=0)return false; out.resize(wlen); MultiByteToWideChar(CP_UTF8,0,in.data(),(int)in.size(),out.data(),wlen); if(!out.empty()&&out[0]==0xFEFF) out.erase(out.begin()); return true;}
...
bool ok=WinHttpSendRequest(r,WINHTTP_NO_ADDITIONAL_HEADERS,0,WINHTTP_NO_REQUEST_DATA,0,0,0)&&WinHttpReceiveResponse(r,nullptr); std::string bytes; while(ok){DWORD avail=0; if(!WinHttpQueryDataAvailable(r,&avail)){ok=false;break;} if(avail==0)break; size_t old=bytes.size(); bytes.resize(old+avail); DWORD got=0; if(!WinHttpReadData(r,bytes.data()+old,avail,&got)){ok=false;break;} bytes.resize(old+got);} WinHttpCloseHandle(r); WinHttpCloseHandle(c); WinHttpCloseHandle(s); if(!ok){err=L"HTTP读取失败"; return false;} return Utf8ToWide(bytes,outText);
}
