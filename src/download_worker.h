...
struct DownloadOptions { int retryCount=3; int dnsTimeoutMs=10000; int connectTimeoutMs=10000; int sendTimeoutMs=30000; int receiveTimeoutMs=30000; bool enableResume=true; };
...
public:
    ~DownloadWorker();
    bool Start(HWND hwnd, int row, int fileIndex, int fileCount, const std::wstring& filename, const std::wstring& url, const std::wstring& savePath, const DownloadOptions& options, UINT msgProgress, UINT msgLog, UINT msgDone);
