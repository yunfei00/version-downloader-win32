...
cl /nologo /utf-8 /std:c++17 /EHsc /O2 /MD /DUNICODE /D_UNICODE /DWIN32 /D_WINDOWS /DNDEBUG /DBUILD_COMMIT=L"%GIT_COMMIT%" /c ^
  src\main.cpp src\app.cpp src\ui.cpp src\csv_parser.cpp src\logger.cpp src\download_worker.cpp src\hash_utils.cpp src\string_utils.cpp src\http_directory_scanner.cpp /Fo:build\
if errorlevel 1 (
...
link /nologo /OUT:build\version-downloader.exe build\main.obj build\app.obj build\ui.obj build\csv_parser.obj build\logger.obj build\download_worker.obj build\hash_utils.obj build\string_utils.obj build\http_directory_scanner.obj build\version.res ^
